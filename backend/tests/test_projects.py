...
def test_create_and_list_projects(auth_client):
    ...
    r = auth_client.post("/projects/", json=payload)
...
def test_update_and_delete_project_authz(auth_client, db_session, test_user):
    ...
    r = auth_client.post("/projects/", json={"name": "P1", "description": "D"})
    pid = r.json()["id"]
    ...
    try:
        ...
        r2 = auth_client.put(f"/projects/{pid}", json={"name": "New"})
        assert r2.status_code == status.HTTP_404_NOT_FOUND

        # Delete should 404 as well
        r3 = auth_client.delete(f"/projects/{pid}")
