...
def _create_project(auth_client):
    r = auth_client.post("/api/projects/", json={"name": "TasksProj", "description": "TD"})
...
def test_task_lifecycle(auth_client):
    ...
    r = auth_client.post(f"/api/project/{project_id}/tasks/", json=task_payload)
    assert r.status_code == status.HTTP_201_CREATED
    created = r.json()
    assert created["title"] == "T1"

    # List
    r2 = auth_client.get(f"/api/project/{project_id}/tasks/")
    ...
    # Get by id
    r3 = auth_client.get(f"/api/project/{project_id}/tasks/{task_id}")
    assert r3.status_code == status.HTTP_200_OK
    assert r3.json()["id"] == task_id

    # Update
    r4 = auth_client.put(
        f"/api/project/{project_id}/tasks/{task_id}",
    ...
    # Delete
    r6 = auth_client.delete(f"/api/project/{project_id}/tasks/{task_id}")
    assert r6.status_code == status.HTTP_204_NO_CONTENT

    # After delete, list should be empty
    r7 = auth_client.get(f"/api/project/{project_id}/tasks/")
